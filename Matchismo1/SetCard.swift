...
//  Matchismo1
//
//  Created by Brooke Frandsen on 9/29/14.
...
class setCard: Card {
    var suit: String! {
        didSet {
            if !contains(setCard.validSuits(), suit) {
                suit = "?"
            }
            contents = "\(setCard.rankStrings()[rank])\(suit)"
        }
    }
    ...
    override func match(otherCards: [Card]!) -> Int {
        ...
        if otherCards.count == 1 {
            if let otherCard = otherCards.last as? setCard {
            ...
            let otherCard1 = otherCards[0] as? setCard
            let otherCard2 = otherCards[1] as? setCard
